...
- **Поддержка Яндекс Карт** наряду с Яндекс Навигатором. Мост читает экранную панель поворотов обоих — `ru.yandex.yandexnavi` и `ru.yandex.yandexmaps` (общие resource-id панели MapKit, определяются по пакету события).
- **Переключатель виджета навигации в приборной панели** на экране настройки. Включает/выключает симулированное широковещание AMap, чтобы пользователь мог отключить его при конфликте с собственным навигационным виджетом автомобиля.
- **Напоминание в приложении**, что в Яндексе нужно включить *Настройки → Навигация → «Показывать подсказки поворотов в углу экрана»* (первый переключатель) — иначе Яндекс рисует подсказку внутри карты и мост не читает повороты.

### Изменено

- **Определение значка поворота теперь «сначала текст, потом пиксели».** Сначала читается `contentDescription` изображения манёвра (детерминированно; заполнено на некоторых сборках Яндекса — так делает OpenBYD); пиксельная классификация применяется только при его отсутствии. Тест на авто подтвердил, что на этой сборке Яндекса `contentDescription` пуст, поэтому пиксельный классификатор остаётся рабочим путём.
- **Значок поворота дебаунсится** (2 последовательных чтения), чтобы один ошибочный кадр не перекидывал стрелку в кластере — исправляет наблюдавшееся мерцание `STRAIGHT⇄LEFT` на статичном повороте.
- **Конец ведения определяется через окно в 4 секунды**, а не по одному пропущенному кадру — предотвращает ложные STOP, когда панель поворота исчезает между экранами.
- Отладочные логи теперь пишутся в публичную папку **Downloads/VoltFlowNav/**, чтобы их можно было открыть через файловый менеджер автомобиля без ADB.
- Настройка через Shizuku опрашивает binder до **12 раз** перед сдачей, чтобы медленный демон после холодной загрузки всё равно привязался.
