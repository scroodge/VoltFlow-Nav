...
Вызов в `MapNotificationListenerService.onNotificationPosted`, метка `:goto_0` после
получения title/text/subText (регистры: v1=this(Context), v6=title, v9=text, v8=subText):
...
(до шагов дедупа/фильтра), дубликаты нормальны для сбора формата.
...
## Структура каталога (lean — decoded/ генерируется, не в git)
    ...
    openbyd-patch/
      ...
      apply_patches.py                правки в decoded/ (идемпотентно)  [tracked]
...
В git только `original/`, `patches/`, два скрипта и этот файл. Остальное регенерируется `build.sh` —
репозиторий остаётся лёгким, логика патча — источник правды.

## Сборка

    ./build.sh            # decode + patch + sign (воспроизводимо)
...
автоматически создаётся при первом запуске (alias `yb`, storepass/keypass `android`).
